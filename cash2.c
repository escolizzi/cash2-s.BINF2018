...
void UpdOrdReset(struct updorder **updorder_pp)
{
  ...
  /*This function was repaired to correctly go through the whole
    array, by indexing on the "testVal" variable, while it was
    previously erroneously done on the "i" variable. The latter case
    does not guaranty shuffling the whole array, resulting in an
    apparent bias in updating towards the lower right of the field
    (e.g. in the case of a growing population, based on more than 1
    neighbor. 23-10-2012 --Thomas, Bram, Rutger*/
  i=(length+7)/8;
  switch(length%8)
    {
    case 0: do { target=(double)testVal+((double)(length-testVal))*genrand_real2();
             if(testVal!=target){//(i!=target){
                    temp=*(*updorder_pp+target);
                    //*(*updorder_pp+target)=*(*updorder_pp+i);
...
MARGOLUS *** MargolusNeigh(void)
{
  ...
  fprintf(stderr,"If you are using MargolusNeigh: Please use even values for nrow and ncol\n");
