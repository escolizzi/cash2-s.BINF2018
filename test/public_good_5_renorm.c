/*PUBLIC GOOD MODEL
 Version 5.0:
 include command line backup file name
*/
...
int GetColorIndexFromNeigh2(TYPE2 **bact,int i, int j);
double ToMovie(TYPE2 **bact);
double ToMovie2(TYPE2 **bact,TYPE2 **good); //notice that for now
...
double cst=4.2;	// production of good from resources: conversion rate = cost
double bnf=10.;	// intake of good from environment: conversion rate = benefit
...
double death=0.2;	//death rate
double kmut=0.0;	//mutation rate
...
int time_save_movie=10;
int time_backup=1000;
int sample_size=10000;
char par_movie_directory_name[BUFSIZE]="moviePBrenorm";
char par_data_file_name[BUFSIZE]="dataPBrenorm.txt";
char par_backup_directory_name[BUFSIZE]="backupPBrenorm";
...
void Initial(void)
{
  ...
  nrow = 729; /* # of row (default=100)*/
  ncol = 729; /* # of column (default=100)*/
  ...
  */
  ...
  OpenPNG(par_movie_directory_name, nrow, 2*ncol);
...
void NextState(int row,int col)
{
  ...
  //death is done outside next state function
}
...
int neicomp[9][9]={0};  // #coop,cheat
//this should decide colors for a neighbourhood, based on proportion of coop, selfish, empty
// bact[i][j] is the upper left pixel
int GetColorIndexFromNeigh2(TYPE2 **bact,int i, int j)
{
  int color;
  double max_fval_we_color;
  int ii,jj;
  int sumc=0,sump=0,sume=0;
  
  //sets the color table for neigh
  ...
  if(firsttime){
    ...
    for(ii=0;ii<9;ii++)for(jj=0;jj<9;jj++){
      neicomp[ii][jj]=ci_comp;
      r=(int)(255*(ii/9.));
      b=(int)(255*(jj/9.));
      ColorRGB(ci_comp,r,0,b);
      ci_comp--;
    }
  }
  
  for(ii=0;ii<3;ii++)for(jj=0;jj<3;jj++){
    if(bact[i+ii][j+jj].val==0) sume++;
    else{ 
      if( bact[i+ii][j+jj].fval< 0.5 ) sump++;
      else sumc++;
    }
  }
...
return neicomp[sumc][sump];  
}




double ToMovie2(TYPE2 **bact,TYPE2 **good)
{
  int popcount=0;
  double avfval=0.;
  //double max_fval_we_color = (k_max*100. - 10.)/100.; /*Remember that colour indexes in the gradient start at 10*/
  ...
  for(i=0; i<=nrow;i++){
    if(NULL==(tomovie[i]=(int *)malloc( 2*(ncol+2) *sizeof(int)))) {
      fprintf(stderr,"SaveMovie(): Error. Memory allocation unsuccessful.\n");
      return -2.;
    }
  }
  ...
  //here you'll save data from neigh
  int ii,jj;
  for(i=1;i<=nrow;i+=3)for(j=1;j<=ncol;j+=3){
    color_index=GetColorIndexFromNeigh2(bact,i,j); //this is function that makes colours from some features...
    for(ii=0;ii<3;ii++)for(jj=0;jj<3;jj++){
      //printf("i=%d, j=%d, ii=%d, jj=%d, j+1+ncol+jj=%d\n",i,j,ii,jj,j+ncol+jj);
      tomovie[i+ii][j+ncol+jj] =color_index;
  ...
  for(i=0; i<nrow+1;i++) free(tomovie[i]);
  free(tomovie);
